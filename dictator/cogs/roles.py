...
class Roles(commands.Cog):
    ...
    async def ncl(self, interaction: discord.Interaction) -> None:
        ...
        if self.already_has_role(interaction, ROLE_1["name"]):
            await interaction.followup.send(f'{interaction.user.mention}, you already have this role!', ephemeral=True)
            return

        if self.playtime_less_than(interaction.user.id, ROLE_1["hours"] * 60):
            await interaction.followup.send(f'{interaction.user.mention}, you do not have {ROLE_1["hours"]} or more hours in game. Nearly!', ephemeral=True)
            return
            
        await self.assign_role(interaction, ROLE_1["name"], "User claimed role")
        await interaction.followup.send(f'Woohoo, {interaction.user.mention}! You have claimed the \'{ROLE_1["name"]}\' role, for playing {ROLE_1["hours"]} or more hours in game! *You\'re starting to know your way around!*')
    ...
    async def exp(self, interaction: discord.Interaction) -> None:
        ...
        if self.playtime_less_than(interaction.user.id, ROLE_2["hours"] * 60):
            await interaction.followup.send(f'{interaction.user.mention}, you do not have {ROLE_2["hours"]} or more hours in game. Keep on playin!', ephemeral=True)
            return

        await self.assign_role(interaction, ROLE_2["name"], "User claimed role")
        await interaction.followup.send(f'Congratulations, {interaction.user.mention}! You have claimed the \'{ROLE_2["name"]}\' role, for playing {ROLE_2["hours"]} or more hours in game! *Go take a break!*')
    ...
    async def vet(self, interaction: discord.Interaction) -> None:
        ...
        if self.playtime_less_than(interaction.user.id, ROLE_3["hours"] * 60):
            await interaction.followup.send(f'{interaction.user.mention}, you do not have {ROLE_3["hours"]} or more hours in game. Surely just a few more to go...!', ephemeral=True)
            return
            
        await self.assign_role(interaction, ROLE_3["name"], "User claimed role")
        await interaction.followup.send(f'Woah, {interaction.user.mention}! You have claimed the \'{ROLE_3["name"]}\' role, for playing {ROLE_3["hours"]} or more hours in game! *You\'re apart of the furniture now*')
    ...
    async def wil(self, interaction: discord.Interaction) -> None:
        ...
        if self.already_has_role(interaction, ROLE_4["name"]):
            await interaction.followup.send(f'{interaction.user.mention}, you already have this role! Doh!', ephemeral=True)
            return

        if self.playtime_less_than(interaction.user.id, ROLE_4["hours"] * 60):
            await interaction.followup.send(f'{interaction.user.mention}, you do not have {ROLE_4["hours"]} or more hours in game. Just around the corner, *right?*', ephemeral=True)
            return
            
        await self.assign_role(interaction, ROLE_4["name"], "User claimed role")
        await interaction.followup.send(f'Woah, {interaction.user.mention}! You have claimed the \'{ROLE_4["name"]}\' role, for playing {ROLE_4["hours"]} or more hours in game! *I suppose you can go now*')
